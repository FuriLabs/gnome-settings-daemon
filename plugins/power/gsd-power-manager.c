...
backlight_enable (GsdPowerManager *manager)
{
        ...
        if (manager->saved_brightness > 0) {
                ...
                gsd_backlight_set_brightness_async (manager->backlight, manager->saved_brightness, FALSE, NULL, NULL, NULL);
                ...
                if (sysfs_percentage <= 0) {
                        ...
                        gsd_backlight_set_brightness_async (manager->backlight, 1, FALSE, NULL, NULL, NULL);
                } else {
                        g_debug ("No saved brightnes value available, setting brightness to the last udev value with percentage %d", sysfs_percentage);
                        gsd_backlight_set_brightness_async (manager->backlight, sysfs_percentage, FALSE, NULL, NULL, NULL);
                }
        } else {
                g_debug ("No backlight value available, setting to 50 percent");
                gsd_backlight_set_brightness_async (manager->backlight, 50, FALSE, NULL, NULL, NULL);
        }
...
backlight_disable (GsdPowerManager *manager)
{
        ...
        gsd_backlight_set_brightness_async (manager->backlight, 0, FALSE, NULL, NULL, NULL);
...
set_backlight_brightness (GsdPowerManager *manager)
{
        ...
        backlight_brightness = gsd_backlight_get_brightness (manager->backlight);
...
gsd_power_manager_startup (GApplication *app)
{
        ...
        */
        if (manager->backlight) {
                int brightness = gsd_backlight_get_brightness (manager->backlight);
                backlight_iface_emit_changed (manager, GSD_POWER_DBUS_INTERFACE_SCREEN,
                                              brightness, NULL);
        } else {
